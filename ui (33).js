...
function buildAccountReportHTML(cliente, mes, anio, rows, pyRows, insight, lang) {
  const initialLang = lang === 'en' ? 'en' : 'es';
...
function render(){
  ...
  html += '<div class="insight-note"><div class="insight-note-text">'+unGenInsight()+'</div></div>';
  ...
  html += '<div class="insight-note"><div class="insight-note-text">'+sectionInsight(catPool,t('nounCat'),0.15,'insightCatNeutral')+'</div></div>';
  ...
  html += '<div class="insight-note"><div class="insight-note-text">'+sectionInsight(famPool,t('nounFam'),0,'insightFamNeutral')+'</div></div>';
  if(famList.length){
  ...
  document.querySelectorAll('.lang-flag-btn').forEach(function(btn){ btn.addEventListener('click', function(){ var l=btn.getAttribute('data-lang'); if(l!==LANG){ LANG=l; render(); } }); });
